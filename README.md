...
- **High Round (최고 라운드)**: 각 던전에서 메시아가 도달한 가장 높은 라운드 기록. 영구 보존되며 영지 HUD에 표시됩니다.
- **Grimoire Inventory (그리모어 인벤토리)**: 모든 유닛(용병/몬스터)이 보유한 전용 금서 슬롯.
- **Territory (영지)**: 메인 메뉴 및 기지
- **Dungeon (던전)**: 일반적인 몬스터 사냥 테마
- **Elite Monster (엘리트 몬스터)**: 일반 몬스터의 강화 버전. 라운드마다 일정 확률(기본 5%, 라운드당 증가)로 출현하며, 크기가 1.2배 크고 스탯이 1.5배 강력함.
- **Arena (아레나)**: 영웅 vs 영웅 대결 테마
- **Raid (레이드)**: 거대 보스(대왕 고블린) 처치 테마
- **Messiah (메시아)**: 플레이어의 화신이자 전장의 관찰자. 게임 내의 모든 재화와 영구적인 권능은 메시아에게 귀속됩니다. (기존 'User' 용어 폐기)
...
- **Melee Projectile System (근접 투사체 시스템) ⚔️**: 모든 근접 평타를 매우 짧은 사거리와 초고속을 가진 투사체(MeleeProjectile)로 처리하는 방식. 원거리와 근접 시스템을 단일화하여 '추가 탄두' 등의 투사체 관련 특성이 모든 공격에 일관적으로 적용되도록 합니다.
- **Shadowmancer (쉐도우맨서)**: 로그의 변형 클래스. 그림자를 자유자재로 다루며, 잠영(Sinking Shadow)을 통해 전장을 누비는 어둠의 마법 전사입니다.
...
### 🚀 특수 시스템 (Special Systems)

#### 1. 유닛-투사체 시스템 (Unit-as-Projectile)
단순한 애니메이션 돌진 대신, 유닛을 `HeroDashProjectile`이라는 특수 투사체 컨테이너에 태워 발사하는 방식입니다.
- **장점**: 경로상 모든 적 타격 판정 용이, 화려한 고정형 궤적 이펙트, 카메라 추적 안정성 확보.
- **주의**: 컨테이너 편입 시 좌표계 변환(`Dummy Initialization`)과 실종 방지용 `isPierce` 설정이 필수입니다.

#### 2. 스킬 시전 보호 (Skill Concurrency Control)
`isBusy` 플래그를 통해 한 유닛이 동시에 여러 광역 기술을 시전하거나 연출이 겹치는 것을 방지합니다.
- 특히 궁극기 시전 후 안정화 시간(0.5초)을 두어 전투의 가독성을 높였습니다.
...
* **직업 명칭 및 성장 특색 (Class Names & Scaling Traits):**
  ...
  * `totemist`: 토템술사 - 마법 공격력(`mAtk`)이 비약적으로 상승하며, 소환물(토템)에 스탯을 100% 전이하여 전투를 보조함.
  * `shadowmancer`: 쉐도우맨서 - 로그의 변형. 물리 공격력(`atk`)과 마법 공격력(`mAtk`)이 조화롭게 상승하며, 높은 기동성과 치명타, 그리고 그림자 마법을 활용함.
...
* **감전 (Shock):** 전술 수류탄 등으로 인한 공격 중단 상태. "평타"만 불가능하며 이동, 스킬, 궁극기 사용은 가능합니다. ⚡ 아이콘으로 표시.
...
* **전기 수류탄 (Electric Grenade):** 레오나의 기술. 지정된 범위에 감전 피해를 입히고 적들을 일시적으로 감전 상태로 만듭니다. ⚡
...
- **가라! 바바오! (Gara! Babao!)**
  - **사용자**: 바오 (`bao`)
  - **효과**: 화면 전체에 염력으로 거대 바위들을 소환하여 낙하시킵니다. 총 **25발**의 바위가 무작위 위치에 떨어져 각각 **마법 공격력의 2.5배** 마법 피해를 입히며, **AOE 반경 150px** 내 모든 적을 타격합니다.
